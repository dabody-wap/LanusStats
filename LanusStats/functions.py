...
def get_proxy():
    ...
    try:
        ...
        # التأكد من وجود عنصر الجدول قبل محاولة استخدامه
        tables = driver.find_elements(By.TAG_NAME, 'table')
        if not tables:
            raise Exception("No table found on the proxy website.")
            
        table = tables[0]
        
        # 1. تخزين محتوى HTML الجدول
        table_html = table.get_attribute('outerHTML')

        # 2. التحقق من أن HTML الجدول ليس None قبل تمريره إلى pd.read_html()
        if table_html is None:
            raise Exception("Outer HTML attribute returned None. Could not read table data.")

        # 3. استخدام المتغير الآمن (table_html)
        df = pd.read_html(table_html)[0]
        
        # تأكد من أن 'Port' موجود
        if 'Port' not in df.columns:
            raise Exception("Port column not found in the scraped data.")
        
        # إصلاح لمشكلة NaN إذا كان هناك اختلاف في أنواع الأعمدة
        ...
        # تأكد من إغلاق المتصفح في حالة حدوث أي خطأ
        try:
            driver.close()
            driver.quit()
        except:
            pass # قد يكون أغلق مسبقاً
        raise e
